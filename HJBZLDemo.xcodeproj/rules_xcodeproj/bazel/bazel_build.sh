...
"$BAZEL_INTEGRATION_DIR/process_bazel_build_log.py" \
  ...
  "$output_groups_flag" \
  "@@rules_xcodeproj~1.6.0~internal~rules_xcodeproj_generated//generator/xcodeproj:xcodeproj" \
...
if [[ -n "${target_ids:-}" ]]; then
  diff_output=$(comm -23 <(printf '%s\n' "${target_ids[@]}") "$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj~1.6.0~internal~rules_xcodeproj_generated/generator/xcodeproj/xcodeproj_target_ids")
